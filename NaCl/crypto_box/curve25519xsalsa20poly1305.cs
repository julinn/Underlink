...
namespace UCIS.NaCl.crypto_box {
	public class curve25519xsalsa20poly1305 {
		...
		static unsafe public int crypto_box_getpublickey(Byte* pk, Byte* sk) {
			return crypto_scalarmult.curve25519.crypto_scalarmult_base(pk, sk);
		}
		...
		static unsafe public int crypto_box_beforenm(Byte* k, Byte* pk, Byte* sk) {
			Byte[] s = new Byte[32];
			fixed (Byte* sp = s, sigmap = sigma) { //, np = n
				crypto_scalarmult.curve25519.crypto_scalarmult(sp, sk, pk);
				return crypto_core.hsalsa20.crypto_core(k, null, sp, sigmap); //k, np, sp, sigmap
			}
		}
		...
		static unsafe public int crypto_box_keypair(out Byte[] pk, out Byte[] sk) {
			sk = new Byte[32];
			pk = new Byte[32];
			randombytes.generate(sk); //randombytes(sk, 32);
			fixed (Byte* skp = sk, pkp = pk) return crypto_scalarmult.curve25519.crypto_scalarmult_base(pkp, skp);
		}
		static unsafe public int crypto_box_getpublickey(out Byte[] pk, Byte[] sk) {
			pk = new Byte[32];
			fixed (Byte* skp = sk, pkp = pk) return crypto_box_getpublickey(pkp, skp);
		}
		static unsafe public int crypto_box_beforenm(Byte[] k, Byte[] pk, Byte[] sk) {
			fixed (Byte* kp = k, pkp = pk, skp = sk) return crypto_box_beforenm(kp, pkp, skp);
		}
		static unsafe public Byte[] crypto_box_beforenm(Byte[] pk, Byte[] sk) {
			Byte[] k = new Byte[BEFORENMBYTES];
			int ret;
			fixed (Byte* kp = k, pkp = pk, skp = sk) ret = crypto_box_beforenm(kp, pkp, skp);
