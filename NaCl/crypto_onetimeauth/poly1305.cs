	...
	unsafe static class poly1305 {
		...
		public static int crypto_onetimeauth_verify(Byte* h, Byte* inv, UInt64 inlen, Byte* k) {
			...
			fixed (Byte* correctp = correct) {
				crypto_onetimeauth(correctp, inv, inlen, k);
				return crypto_verify._16.crypto_verify(h, correctp);
			}
		}
		...
		public static int crypto_onetimeauth(Byte* outv, Byte* inv, UInt64 inlen, Byte* k) {
			UInt32 j;
